 ...
 * 		 fail 
 * 
 * 	Cilent --- Security Filter -- Authentication -- Authorization - Controller
 * 				 사용자 정의								   |
 * 														Session/Cookie
 * 														   |	  |
 * 														일반보안   JWT
...
public class SecurityConfig{
	...
	JdbcUserDetailsManager jdbcUserDetailsManager() {
		...
		manager.setUsersByUsernameQuery("SELECT userid as username,userpwd as password,enabled FROM project_member_2 WHERE userid=?");
		manager.setAuthoritiesByUsernameQuery("SELECT userid as username, authority FROM authority_2 WHERE userid=?");
		return manager;
	}
